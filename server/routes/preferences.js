...
router.get('/', async (req, res) => {
  res.json({ preferences: await store.getPreferences() });
});
...
router.put('/', async (req, res) => {
  ...
  const { preferences } = await store.updatePreferences(changes, {
    updatedBy: 'user',
...
router.get('/history', async (req, res) => {
  res.json({ changes: await store.listPreferenceChanges(20) });
});
